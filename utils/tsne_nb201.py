...
# Architecture data
architecture_data = json.load(open("data/zc_nasbench201_layerwise.json", "r"))['cifar10']
...
for idx, zc_name in tqdm(enumerate(zc_name_list)):
    ...
    # Append the t-SNE embedded data to the results list with labels
    for point, label in zip(embedded_data, zc_values.keys()):
        # embedded_data shape (15625, 2) -> x1 (15625,), x2 (15625,)
        x1 = embedded_data[:, 0]
        x2 = embedded_data[:, 1]
        tsne_results[zc_name] = {'x1': x1.tolist(), 'x2': x2.tolist(), 'color': zc_color_list[idx]}
...
with open('./data/tsne_nb201_output.json', 'w') as f:
